...
GMFN double SetSceneNodeOrientation(double scene_node_ptr, double yaw, double pitch, double roll)
{
   ...
   scene_node->setOrientation(Euler(Ogre::Radian(Ogre::Degree(yaw)), Ogre::Radian(Ogre::Degree(pitch)), Ogre::Radian(Ogre::Degree(roll))));
...
GMFN double GetSceneNodeOrientation(double scene_node_ptr)
{
   ...
   if (mEulerYaw != NULL)
   {
      *mEulerYaw = quat.getYaw().valueDegrees();
