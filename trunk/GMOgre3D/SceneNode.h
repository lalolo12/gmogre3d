...
GMFN double CreateRootChildSceneNode(double x, double z, double y)
{
   if (mSceneMgr == NULL)
      return FALSE;

   Ogre::SceneNode *node = mSceneMgr->getRootSceneNode()->createChildSceneNode(Ogre::Vector3(x, y, z));
   
   return ConvertToGMPointer(node);
}


GMFN double CreateChildSceneNode(double scene_node_ptr, double x, double z, double y)
{
   ...
   Ogre::SceneNode *child_node = scene_node->createChildSceneNode(Ogre::Vector3(x, y, z));
...
GMFN double GetSceneNodeScale(double scene_node_ptr)
{
   ...
   *mVectorResultX = vec.x;
   *mVectorResultY = vec.z;
   *mVectorResultZ = vec.y;
...
GMFN double SetSceneNodeOrientation(double scene_node_ptr, double w, double x, double z, double y)
{
   Ogre::SceneNode *scene_node = ConvertFromGMPointer<Ogre::SceneNode*>(scene_node_ptr);
   ...
   scene_node->setOrientation(w, z, y, z);
...
GMFN double GetSceneNodeOrientation(double scene_node_ptr)
{
   ...
   *mQuaternionResultY = quat.y;
   *mQuaternionResultZ = quat.z;
