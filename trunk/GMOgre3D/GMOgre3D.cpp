...
GMFN double StartEngine(double render_engine, double hwnd, double window_width, double window_height)
{
   try
   {
      ...
      // Setup the links to our GM vector/quaternion result variables
      mVectorResultX = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("vector_resx"));
      mVectorResultY = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("vector_resy"));
      mVectorResultZ = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("vector_resz"));

      mQuaternionResultW = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("quaternion_resw"));
      mQuaternionResultX = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("quaternion_resx"));
      mQuaternionResultY = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("quaternion_resy"));
      mQuaternionResultZ = mGMAPI->GetGlobalVariablePtr(mGMAPI->GetSymbolID("quaternion_resz"));
   }
   catch(Ogre::Exception& e)
   {
      DisplayError(e.what());
...
GMFN double RenderFrame()
{
   ...
   while (iter != end_iter)
   {
      ...
      {
         double ogre_yaw = gminstance->direction;
         if (ogre_yaw < -90 && ogre_yaw > -180)
	         ogre_yaw = -180 + ogre_yaw - 270;
         else
	         ogre_yaw = ogre_yaw - 90;
         node->setOrientation(Ogre::Quaternion(Ogre::Radian(Ogre::Degree(ogre_yaw)), Ogre::Vector3::UNIT_Y) * Ogre::Quaternion(Ogre::Radian(Ogre::Degree(*gminst.pPitch)), Ogre::Vector3::UNIT_X) * Ogre::Quaternion(Ogre::Radian(Ogre::Degree(*gminst.pRoll)), Ogre::Vector3::UNIT_Z));
